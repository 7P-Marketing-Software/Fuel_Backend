...
class PTSWebSocketHandler
{
    ...
    {
        $this->dataProcessor = new PTSDataProcessor();
    ...
    public function onClose(TcpConnection $connection)
    {
        ...
        if ($ptsId !== 'unknown') {
            WebSocketSession::create([
                ...
                'handshake_headers' => $session['headers'],
                'connected_at' => $session['connected_at'],
                'disconnected_at' => Carbon::now(),
                'messages_received' => $session['messages_received'] ?? 0,
            ]);
        }
    ...
    private function processPTSMessage(array $data, TcpConnection $connection)
    {
        ...
        $log = PTSLog::create([
            ...
            'method' => 'WEBSOCKET',
            'uri' => '/pts-websocket',
            'headers' => $this->sessions[$connection->id]['headers'] ?? [],
            'body' => $data,
            'ip_address' => $connection->getRemoteIp(),
